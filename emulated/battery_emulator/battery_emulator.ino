#define NET_ID 01 //network ID, change as needed !!MAKE SURE NO TWO DEVICES HAVE THE SAME!!
...
void loop() {
  do {
    ...
    cur_raw = analogRead(A1);

    //current conversion, takes a bit more effort because is -50 to +0
    if (cur_raw > 523) {
      cur = cur_raw - 523;
      cur = 0 - (cur / 10.0);
    }
...
//voltage is more straight forward...
    volt = 16.0 - ((volt_raw / 1023.0) * 8.0);
    delay(5);
