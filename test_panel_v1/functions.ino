void RGB_DISP() {
  ...
  PORTB = PORTB | 0X0E; //sets the RGB pins to high
  PORTD = PORTD & 0X03; //set all the position pins to low
  PORTB = PORTB & 0xFE; //there's one here too
  temp = rgb_val[color]; //get the color value
  temp = temp & ( blk_map | BLK); //used for flashing
  temp2 = temp >> 6 & 0x01; //get the single bit we need here XD
  temp = temp << 2; //align the value to the register
  PORTD = PORTD | temp;  //write the values
  PORTB = PORTB | temp2; //write the values
